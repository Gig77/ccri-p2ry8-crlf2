...
for (p in names(input)) {
  ...
  #xeno.col <- input[[p]]$timepoints[grepl("xeno", input[[p]]$identifiers)]
  ...
  data.patient <- data.patient[conserved | (primary & cosmic),]
  ...
  group.notdia <- data.patient[data.patient$D == 0 & data.patient$Dxg == 0 & data.patient$R > 0.2 & data.patient$Rxg > 0.2 & data.patient$RR > 0.2,]
    ...
    # label genes conserved in at least two samples (all xenografts counted as one)
  #genes.to.label <- unique(data.patient$gene[rowSums(apply(data.patient[,input[[p]]$timepoints], 2, function(x) x > 0)) > 1])
  
  # plot

  data.patient$group <- "unassigned"
  #data.patient$group[rowSums(apply(data.patient[,input[[p]]$timepoints], 2, function(x) x > 0))>=5] <- "conserved"
  m <- melt(data.patient, id.vars=c(cols.merge, "group"))
  m$mutation <- paste0(m$chr, ":", m$pos, ":", m$ref, ">", m$alt)
  m$color <- as.integer(as.factor(as.character(m$gene)))
  
  print(xyplot(value ~ variable | gene, 
               group=mutation, 
               data=m[m$group=="unassigned",], 
               type='b', 
               par.settings=simpleTheme(pch=ifelse(m$deleterious=="yes", 19, 1)),
               main=p, 
               as.table=TRUE, 
...
# draw lines and gene names
#par(mfrow=c(nrow(data.patient), 1), mar=c(0,3,0,1))
for(i in 1:nrow(data.patient)) {
  #plot(0, 0, xlim=c(1, 1.5*(num.tp-1)+1.6), ylim=c(0, input[[p]]$max.af.plot), type="n", xaxt="n", yaxt="n", xlab="", ylab="Allelic frequency", main=ifelse(i==1, paste(p, " (n=", nrow(data.patient), ")", sep=""), ""))
  #if(i==nrow(data.patient)) {
  #  axis(1, at = x, labels=paste0(input[[p]]$timepoints, "\n(", input[[p]]$blast.counts, "% blasts)"), padj=0.5)
  #}
  #axis(2, at = seq(0, 1, 0.1), las = 1) 
  
  lines(x, data.patient[i, (ncol(data.patient)-num.tp+1):ncol(data.patient)], type="b", col=i%%20, lwd=ifelse(data.patient$deleterious[i] == "yes", 4, 1))
  
  if (data.patient$gene[i] != "" & (data.patient$deleterious[i] == "yes" | data.patient$gene[i] %in% input[[p]]$genes.to.label)) {
    text(1.25, data.patient[i, ncol(data.patient)-num.tp+1], paste0(data.patient$gene[i], " --"), adj=c(1, 0.5), cex=0.8)
    text(1.5*(num.tp-1)+1.35, data.patient[i, ncol(data.patient)], paste0("-- ", data.patient$gene[i]), adj=c(0, 0.5), cex=0.8)
  }
}	
