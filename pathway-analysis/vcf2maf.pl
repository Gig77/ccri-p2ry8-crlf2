...
{
	$sample_normal_vcf = $patient2sample{$sample_normal_vcf} ? $patient2sample{$sample_normal_vcf} : $sample_normal_vcf; 
}
if ($sample_tumor_vcf ne $samples[0] and $sample_tumor_vcf ne $samples[1])
{
	$sample_tumor_vcf = $patient2sample{$sample_tumor_vcf} ? $patient2sample{$sample_tumor_vcf} : $sample_tumor_vcf; 
}
...
sub get_impacted_genes
{
	...
	foreach my $eff (split(",", $effs))
	{
		...
		# if multiple transcript of the same genes are impacted, remember the one with the higher impact 
		next if ($tmp{$gene_name}{'impact'} and ($impact eq "LOW" || $impact eq "MODIFIER"));  
		next if ($tmp{$gene_name}{'impact'} and $tmp{$gene_name}{'impact'} eq "HIGH");  
